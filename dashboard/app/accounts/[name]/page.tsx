...
function sourceBadge(source: string | null, sourceId?: string | null) {
  if (source === 'demodesk') return <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-purple-100 text-purple-700">Meeting call</span>
  if (sourceId?.includes('nps')) return <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-orange-100 text-orange-700">NPS</span>
  if (source === 'manual') return <span className="text-xs font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-500">Manual</span>
...
function StatusEditor({ frId, status, onChange }: { frId: string; status: string; onChange: (id: string, val: string) => void }) {
  ...
  if (!editing) {
    return (
      <button onClick={() => setEditing(true)} className={`text-xs font-medium px-2 py-0.5 rounded-full cursor-pointer hover:opacity-80 ${statusColor(status)}`} title="Click to edit">
  ...
  return (
    <div className="relative">
      <input ref={inputRef} value={draft} onChange={(e) => setDraft(e.target.value)}
        onBlur={(e) => { if (!e.relatedTarget?.closest?.('[data-preset]')) commit(draft) }}
        onKeyDown={(e) => { if (e.key === 'Enter') commit(draft); if (e.key === 'Escape') { setEditing(false); setDraft(status) } }}
        className="text-xs px-2 py-0.5 rounded-lg border border-indigo-300 focus:outline-none focus:ring-2 focus:ring-indigo-300 w-32"
      ...
      <div className="absolute top-full mt-1 right-0 bg-white border border-gray-200 rounded-lg shadow-lg z-10 py-1 min-w-max">
        {PRESET_STATUSES.map((p) => (
          <button key={p} data-preset="true" onMouseDown={(e) => { e.preventDefault(); commit(p) }}
            className="block w-full text-left px-3 py-1.5 text-xs hover:bg-gray-50 text-gray-700">{p}</button>
        ))}
      </div>
    </div>
  )
...
export default function AccountPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  const handleAddFR = async () => {
    ...
    if (res.ok) {
      setFrs((prev) => [{ id: '', title: newFRTitle.trim(), status: 'pending', source: 'manual', source_id: 'manual', signal_date: new Date().toISOString() }, ...prev])
  ...
  const openMergeEditor = () => {
    const [a, b] = mergeSelected.map((id) => cards.find((c) => c.id === id))
    if (!a || !b) return
    setMergeBody(`${a.body}\n\n${b.body}`)
  ...
  const confirmMerge = async () => {
    ...
    await fetch('/api/insight-cards/merge', {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center gap-4">
          <Link href="/" className="text-gray-500 hover:text-gray-800 flex items-center gap-1 text-sm font-medium">← Back</Link>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8 space-y-8">
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            ...
            {cards.length >= 2 && (
              <button onClick={() => { setMergeMode(!mergeMode); setMergeSelected([]); setShowMergeEditor(false) }}
                ...
                {mergeMode ? 'Cancel merge' : 'Merge insights'}
              </button>
            )}
          </div>
          {mergeMode && <p className="text-xs text-gray-500 mb-3">{mergeSelected.length === 0 ? 'Select 2 insights to merge.' : mergeSelected.length === 1 ? 'Select one more.' : 'Ready — click Merge selected.'}</p>}

          {cards.length === 0 ? <p className="text-sm text-gray-400">No pending insights.</p> : (
            <div className="space-y-3">
              {cards.map((card) => (
                <div key={card.id} className={`bg-white rounded-xl border p-5 transition-colors ${mergeMode && mergeSelected.includes(card.id) ? 'border-indigo-400 ring-2 ring-indigo-200' : 'border-gray-200'}`}>
                  <div className="flex items-start gap-3">
                    {mergeMode && <input type="checkbox" checked={mergeSelected.includes(card.id)} onChange={() => toggleMergeSelect(card.id)} className="mt-1 h-4 w-4 rounded border-gray-300 text-indigo-600 cursor-pointer flex-shrink-0" />}
                    <div className="flex-1">
                      ...
                      {!mergeMode && (
                        <div className="flex gap-2">
                          <button onClick={() => handleInsightAction(card.id, 'approved')} className="px-3 py-1.5 text-xs font-medium bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">Approve</button>
                          <button onClick={() => handleInsightAction(card.id, 'dismissed')} className="px-3 py-1.5 text-xs font-medium bg-white text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">Dismiss</button>
                        </div>
                      )}
                    </div>
                  </div>
          ...
          {mergeMode && mergeSelected.length === 2 && !showMergeEditor && (
            <button onClick={openMergeEditor} className="mt-3 px-4 py-2 text-sm font-medium bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-colors">Merge selected →</button>
          )}

          {showMergeEditor && (
            <div className="mt-4 bg-white border border-indigo-200 rounded-xl p-5">
              <p className="text-sm font-semibold text-gray-700 mb-2">Edit merged insight</p>
              ...
              <div className="flex gap-2 mt-3">
                <button onClick={confirmMerge} disabled={merging} className="px-4 py-1.5 text-sm font-medium bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors">
                  {merging ? 'Merging…' : 'Confirm merge'}
                </button>
                <button onClick={() => { setShowMergeEditor(false); setMergeSelected([]) }} className="px-4 py-1.5 text-sm font-medium bg-white text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">Cancel</button>
              </div>
            </div>
          )}
        </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide">
              Feature Requests{frs.length > 0 && <span className="ml-2 text-gray-400 font-normal normal-case text-xs">({frs.length})</span>}
            </h2>
            <button onClick={() => setShowAddFR(!showAddFR)} className="text-xs font-medium px-3 py-1 rounded-lg border bg-white text-gray-600 border-gray-200 hover:border-indigo-300 transition-colors">
          ...
          {showAddFR && (
            <div className="bg-white rounded-xl border border-indigo-200 p-4 mb-3">
              ...
              <div className="flex gap-2">
                <button onClick={handleAddFR} disabled={!newFRTitle.trim() || addingFR} className="px-4 py-1.5 text-sm font-medium bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 disabled:opacity-50 transition-colors">
                  {addingFR ? 'Adding…' : 'Add'}
                </button>
                <button onClick={() => { setShowAddFR(false); setNewFRTitle(''); setNewFRNote('') }} className="px-4 py-1.5 text-sm font-medium bg-white text-gray-600 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">Cancel</button>
              </div>
            </div>
          )}

          {frs.length === 0 ? <p className="text-sm text-gray-400">No feature requests recorded.</p> : (
                ...
                <div key={fr.id || fr.title} className="bg-white rounded-xl border border-gray-200 p-4 flex items-center gap-4">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {fr.id
                      ...
                      : <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${statusColor(fr.status)}`}>{fr.status}</span>
                    }
                  </div>
                </div>
              ))}
